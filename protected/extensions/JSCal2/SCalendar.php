...
/*** * @param integer $step */
  ...
  // Get the resources path
    $resources = dirname(__FILE__).DIRECTORY_SEPARATOR.'src';

    // publish the files
    $baseUrl = Yii::app()->assetManager->publish($resources);

    // register the files
    Yii::app()->clientScript->registerScriptFile($baseUrl.'/js/jscal2.js');
    if(is_file($resources.'/js/lang/'.$this->_language.'.js')) {
      Yii::app()->clientScript->registerScriptFile($baseUrl.'/js/lang/'.$this->_language.'.js');
    } else {
      Yii::app()->clientScript->registerScriptFile($baseUrl.'/js/lang/en.js');
    }

    // If skin
	Yii::app()->clientScript->registerCssFile($baseUrl.'/css/jscal2.css');
	Yii::app()->clientScript->registerCssFile($baseUrl.'/css/border-radius.css');
	$skin_url ='/css/'.$this->_skin.'/'.$this->_skin.'.css';
	if(is_file($resources.$skin_url)) {
		Yii::app()->clientScript->registerCssFile($baseUrl.$skin_url);
	}
  }
  ...
  private function createMarkup() {
    ...
    if(isset ($this->_trigger))        $prop .= "trigger          : '$this->_trigger',\n";
    if(isset ($this->_min))				$prop .= "min          : $this->_min,\n";
    if(isset ($this->_max))        $prop .= "max          : $this->_max,\n";
    ...
    if(isset ($this->_showTime))     $prop .= "showTime       :  $this->_showTime,\n";
