...
class TreeBehavior extends CNestedSetBehavior
{
     ...
     * it's brother.
     * @param CActiveTreeRecord $node The node to which you want to append this node.
    ...
    public function deleteNode($deleteChildren = false)
    {
        ...
        if($this->getLeftValue() == null || $this->getRightValue() == null || $this->getLevelValue() == null)
        {
     ...
     */
    public function moveBelow($node)
    {
        return $this->moveNode($node,false,true);
    }
    ...
    private function moveNode ($sibling, $after = false, $aschild = false)
    {
        ...
        // Moving down!
        {
            ...
            if ($after)
            {
                ...
                $move  = $sibling->getRightValue() - $movenode->getRightValue();
            }
            else // before
            {
                $upper = $sibling->getLeftValue() - 1;
                $move  = $sibling->getLeftValue() - $movenode->getRightValue() - 1;
            }
            // sibling or child
            $upper += $aschild;
            // direction
        ...
        // Going up!
        {
            ...
            else // before
            {
                ...
                $move  = $movenode->getLeftValue() - $sibling->getLeftValue();
            }
            // sibling or child
            $lower += $aschild;
            // direction
