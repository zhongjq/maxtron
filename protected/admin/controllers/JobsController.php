...
class JobsController extends BaseController
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update', 'resume'),
				'users'=>array('@'),
			),
	...
	public function actionResume(){
		...
		$pages=new CPagination(JobsResume::model()->count($criteria));
		$pages->pageSize=self::PAGE_SIZE;
		$pages->applyLimit($criteria);

		$sort=new CSort('JobsResume');
		$sort->applyOrder($criteria);

		$models=JobsResume::model()->findAll($criteria);
	...
	{
		$this->render('view',array('model'=> JobsResume::model()->findByPk($_GET['id'])));
	}
	...
	protected function processAdminCommand()
	{
		...
		if(isset($_POST['command'], $_POST['id']) && $_POST['command']==='deleteResume')
		{
			JobsResume::model()->findByPk($_POST['id'])->delete();
			// reload the current page to avoid duplicated delete actions
			$this->refresh();
		}
		if(isset($_POST['command'], $_POST["JobsResume"]['id']) && $_POST['command']==='deleteSelectedResume')
		{
		    foreach ($_POST["JobsResume"]['id'] as $id){
		        if (!empty($id)){
    		        JobsResume::model()->findByPk($id)->delete();
		        }
		    }
			// reload the current page to avoid duplicated delete actions
			$this->refresh();
